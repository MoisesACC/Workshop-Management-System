...
import { Menu, X, Hammer, User, Calendar as CalendarIcon, Phone, LogOut } from 'lucide-react';
        ...
        <nav className="fixed w-full z-50 top-0 start-0 border-b border-white/5 bg-background-dark/80 backdrop-blur-xl transition-all duration-300">
            <div className="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-6 py-2 md:py-3">
                ...
                <NavLink to="/" className="flex items-center gap-2 group shrink-0">
                    <img src="/assets/img/logo.webp" alt="Factoría La Caravana" className="h-12 md:h-16 w-auto group-hover:scale-105 transition-transform" />
                </NavLink>
                    ...
                    {user ? (
                        <div className="flex items-center gap-3">
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">{user.fullName || user.username || user.name || "Mi Perfil"}</span>
                            </NavLink>
                            <button
                                onClick={logout}
                                title="Cerrar Sesión"
                                className="p-2 text-gray-400 hover:text-primary hover:bg-white/5 rounded-xl transition-all active:scale-95 border border-transparent hover:border-white/5"
                            >
                                <LogOut size={20} />
                            </button>
                        </div>
